...
1. **API mode (preferred)** — when ``WEB3_CAREER_API_KEY`` is set in the
   ...
   ``web3.career/web3-jobs-api``). Quirks of the response shape:
       - The body is a mixed-type JSON array; the jobs live inside the
         first list-of-dicts within the root array (per the official
         docs). We scan defensively for that.
       - Field names are not strictly schemed; we read ``title`` /
         ``company`` / ``location`` / ``description`` / ``apply_url``
         (or ``url``) / ``salary``, with fallbacks for common alternates.
...
def _find_jobs_array(payload):
    ...
    Per the official docs the root is shaped like ``[meta, meta, [job, ...]]``;
