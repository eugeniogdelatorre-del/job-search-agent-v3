...
def poll_batch(
    ...
    *,
    ...
    """Block until ``processing_status == 'ended'`` or we time out.

    Transient retrieve errors (network blips, 5xx) are logged and retried;
    they don't abort. We rely on the deadline to bound the loop. If we
    time out the next cron run picks up the work — by design.
    """
...
def extract_json(text: str) -> dict | None:
    ...
    3. As a last resort, regex out the first ``{...}`` block (handles
       responses where the model adds an apology before/after the JSON).
